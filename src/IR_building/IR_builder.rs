...
use crate::compile::{
    BinaryOperatorType, CheckedExpression, FunctionDeclare, FunctionType, LiteralExpr, RawType,
    UnaryOperatorType,
};
...
impl<'ctx> IRBuilder<'ctx> {
    ...
    pub fn import_lib_func(&self) {
        ...
        self.symbol_table.insert(
            "printf".to_string(),
            printf_func.as_global_value().as_pointer_value(),
        );
        self.symbol_table.insert(
            "scanf".to_string(),
            scanf_func.as_global_value().as_pointer_value(),
        );
    }
    ...
    fn build_global_variable(
        ...
        let LLVM_Type = self.llvm_type_from(_type.clone());
    ...
    fn build_function_declare(&self, name: Token, function: FunctionDeclare) {
        let LLVM_function_type = {
            ...
            for param in function._type.param_types.clone() {
                let t = BasicMetadataTypeEnum::from(self.llvm_type_from(param));
                types.push(t);
            }
            let fun_type = self
    ...
    fn build_func_body(&self, function_declare: FunctionDeclare) {
        ...
        for (i, param) in function_declare.param_names.iter().enumerate() {
            ...
            let alloca = self
                .builder
                .build_alloca(
                    self.llvm_type_from(function_declare._type.param_types[i].clone()),
                    &param,
                )
                .unwrap();
            self.builder
    ...
    fn build_any_value(&self, expression: CheckedExpression) -> Box<dyn AnyValue + '_> {
        ...
        match expression {
            ...
            CheckedExpression::VariableName { name } => {
                let var = self
                    .symbol_table
                    .get(&name.text)
            ...
            CheckedExpression::Unary { op, operand } => match (op.operator_type, op.operand_type) {
                ...
                (UnaryOperatorType::AddressOf, _) => {
                    Box::from(if let CheckedExpression::VariableName { name } = *operand {
                        ...
                        panic!("can only take address of variable");
                    })
                }
                _ => todo!(
                    "{}{:#?}",
            ...
            CheckedExpression::Binary { op, left, right } => {
                match (op.clone().left_type, op.clone().right_type) {
    ...
    fn build_aim(&self, aim: CheckedExpression) -> Box<BasicValueEnum> {
        match aim {
            CheckedExpression::VariableName { name } => {
                let var = self
                    .symbol_table
    ...
    fn build_var_declare(
        ...
        let alloca = match _type {
            ...
            RawType::Pointer { inner_type } => self.builder.build_alloca(
                self.llvm_type_from(*inner_type)
    ...
    fn llvm_type_from(&self, _type: RawType) -> BasicTypeEnum<'ctx> {
        match _type {
            ...
            RawType::Pointer { inner_type } => self
                .llvm_type_from(*inner_type)
                .ptr_type(AddressSpace::default())
                .as_basic_type_enum(),
            RawType::Array { inner_type, size } => self
                .llvm_type_from(*inner_type)
